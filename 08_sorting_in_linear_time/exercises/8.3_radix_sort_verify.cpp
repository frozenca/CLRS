...
int main() {
  ...
  {
    vector<uint16_t> A{27, 17, 3, 16, 13, 10, 1, 5, 7, 12, 4, 8, 9, 0};
    fc::radix_sort(A, 5);
    fc::print(A);
  ...
  constexpr int max_k = 30'000;
  constexpr int num_trials = 10'000;

  mt19937 gen(random_device{}());
  uniform_int_distribution<uint16_t> dist(0, max_k);
...

  {
    ...
    auto stat = fc::get_statistics(durations);
    fc::log("Radix sort Time to process a range of {:6} elements:\n"
            "Average : {:10.4f} us,\n"
            "Stdev   : {:10.4f} us,\n"
            "95%     : {:10.4f} us,\n"
            "99%     : {:10.4f} us,\n"
