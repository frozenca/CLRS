...
void tree_test(const Alloc &alloc, bool warmup = false) {
  constexpr int max_n = 5'000'000;
...
int main() {
  ...
  constexpr fc::index_t t = 1024;

  fc::MemoryMappedFile mm_file("database.bin", 1UL << 31UL);
  fc::MemoryResourceFixed<int, 2 * t - 1> mem_res(
      reinterpret_cast<unsigned char *>(mm_file.data()), mm_file.size());
  fc::AllocatorFixed<int, 2 * t - 1> alloc_fixed(&mem_res);

  allocator<int> def_alloc;

  tree_test<set<int>, allocator<int>>(def_alloc,
                                         true); // warm up for benchmarking

  cout << "Warming up complete...\n";
  cout << "frozenca::BTreeSet<int> test\n";
  tree_test<fc::DiskBTree<int, t>, fc::AllocatorFixed<int, 2 * t - 1>>(alloc_fixed);
