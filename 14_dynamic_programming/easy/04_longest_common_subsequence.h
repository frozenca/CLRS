...
void print_lcs_impl(const vector<vector<Direction>> &b, const vector<char> &X,
                    index_t i, index_t j) {
  if (i == 0 || j == 0) {
    return; // the LCS has length 0
  }
  if (b[i - 1][j - 1] == Direction::Upleft) {
    print_lcs_impl(b, X, i - 1, j - 1);
    cout << X[i - 1];
  } else if (b[i - 1][j - 1] == Direction::Up) {
