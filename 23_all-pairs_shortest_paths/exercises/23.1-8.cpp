...
int main() {
  ...
  weight[{4, 3}] = -5;
  weight[{5, 4}] = 6;
  fc::apsp_path(g, weight);
  auto &shortest_lengths =
      g.get_edge_property<int>(fc::GraphPropertyTag::EdgeAPSP);
  cout << "All-pair shortest path lengths:\n";
  for (const auto &[e, w] : shortest_lengths.get()) {
    cout << e.first << '-' << e.second << ": " << w << '\n';
  }
  auto &paths =
      g.get_edge_property<vector<int>>(fc::GraphPropertyTag::EdgePath);
  cout << "Shortest paths:\n";
  for (const auto &[e, path] : paths.get()) {
